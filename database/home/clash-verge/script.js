const main = config => {
    ...
    const hongkongProxyNames = allProxyNames.filter(
        proxyName =>
            proxyName.startsWith("🇭🇰") &&
            !proxyName.toLowerCase().includes("direct"),
    )
    const hongkongAutoProxyGroup = buildAutoProxyGroup(
        "Hong Kong",
        hongkongProxyNames,
    )
    const taiwanProxyNames = allProxyNames.filter(proxyName =>
        proxyName.startsWith("🇹🇼"),
    )
    ...
    const fixedBaseRules = config.rules
        .map(rule =>
            rule.endsWith(",Alink") ? `${rule.slice(0, -5)}Meta` : rule,
        )
        .map(rule =>
            rule.endsWith(",Alink,no-resolve")
                ? `${rule.slice(0, -16)}Meta,no-resolve`
                : rule,
        )
    const newRules = [
